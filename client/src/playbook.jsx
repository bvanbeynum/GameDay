...
class PlayBook extends Component {
	...
	copyPlayBook = () => {
		fetch(`/api/playbooksave?copyid=${ this.state.selectedPlayBook.id }`, { method: "post", headers: {"Content-Type": "application/json"} })
			.then(response => {
				if (response.ok) {
					return response.json();
				}
			...
			.then(data => {
				this.setState(({ playBooks }) => ({ 
					playBooks: [ 
						...
						{
							...data.playBook,
							plays: data.playBook ? data.playBook.plays.map(playBookPlay => ({
									...this.state.plays.find(play => playBookPlay.playId === play.id),
									sort: playBookPlay.sort
								}))
								: []
						}
					...
					this.setState(({ playBooks }) => ({
						selectedPlayBook: playBooks.find(playBook => playBook.id === data.playBook.id),
						toast: { text: "Play book copied", type: "info" } 
					}))
				});
			})
			.catch(error => {
				console.warn(error);
				this.setState({ toast: { text: "Error copying playbook", type: "error" } });
			});
	}
			...
			<div className="playBookPage">
				...
				: this.state.selectedPlayBook ?
					...
					<div className="playBookActions">
						...
						<div className="playBookAction" onClick={ () => { this.copyPlayBook() }}>
							{/* Copy */}
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
